...
class Processor(Initializer):
    ...
    def convert_to_onnx(self):
        pretrained_model = '/data/pulkit/train/EfficientGCNv1/workdir/2003_EfficientGCN-B0_ntu-xsub120/2024-01-18 12-46-34/2003_EfficientGCN-B0_ntu-xsub120.pth.tar'
        checkpoint = torch.load(pretrained_model, map_location=torch.device('cpu'))
        self.model.load_state_dict(checkpoint['model'])
        self.model.to('cpu')

        sample_input = torch.randn((16, 3, 4, 288, 14, 2)).float()
    
        input_names = [ "input" ] 
        output_names = [ "output"]

        ONNX_FILE_PATH = '/data/pulkit/train/EfficientGCNv1/workdir/2003_EfficientGCN-B0_ntu-xsub120/2024-01-18 12-46-34/model.onnx'
        dynamic_axes = {"input":{0:"batch_size"}, "output":{0:"batch_size"}}
        torch.onnx.export(self.model, sample_input, ONNX_FILE_PATH, input_names=input_names, output_names=output_names, dynamic_axes=dynamic_axes, export_params=True)
        #/usr/src/tensorrt/bin/trtexec --explicitBatch --onnx=model.onnx --minShapes=input:1x3x224x224 --optShapes=input:8x3x224x224 --maxShapes=input:16x3x224x224 --shapes=input:3x3x224x224 --saveEngine=model.plan